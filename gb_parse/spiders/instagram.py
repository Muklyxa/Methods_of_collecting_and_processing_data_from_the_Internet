...
class InstagramSpider(scrapy.Spider):
    ...
    def __init__(self, login, enc_password, tags, users, *args, **kwargs):
        self.tags = tags
        self.users = users
        self.login = login
    ...
    def user_page_parse(self, response):
        ...
        yield from self.get_api_follower_request(response, user_data)
    ...
    def get_api_follower_request(self, response, user_data, variables=None):
        if not variables:
            variables = {
                'id': user_data['id'],
                'first': 100,
        ...
        yield response.follow(url, callback=self.get_api_follower, cb_kwargs={'user_data': user_data})
    ...
    def get_api_follower(self, response, user_data):
        if b'application/json' in response.headers['Content-Type']:
            data = response.json()
            yield from self.get_follower_item(user_data, data['data']['user']['edge_followed_by']['edges'])
            if data['data']['user']['edge_followed_by']['page_info']['has_next_page']:
                ...
                yield from self.get_api_follower_request(response, user_data, variables)
    ...
    def get_follower_item(self, user_data, follower_users_data):
